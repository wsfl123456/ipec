...
import { reqNewLogin } from "@utils/api";
...
class LoginStore extends BaseStore {
  ...
  async doLogin(params) {
    ...
    const { errorCode, result = {}, errorMessage }: any = await reqNewLogin(params);
    // const { errorCode, result = {}, errorMessage }: any = await reqLogin({params});
    if (errorCode === "200") {
      if (result.errorCode === 200) {
        if (result.data.promptPerfectInformation) {
          localStorage.setItem("promptPerfectInformation", "show")
        }
        if (remember) {
